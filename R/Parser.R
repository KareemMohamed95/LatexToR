  ...
  public = list(
    ...
    implicit_multiplication = function() {
      skip = c('MINUS', 'PLUS', 'STAR', 'SLASH', 'CARET', 'UNDERSCORE',
               'RIGHT_PAREN', "LEFT_SQUARE", "RIGHT_SQUARE", 'RIGHT_BRACE', 
                'EQUAL', 'PM', 'COMMA', 'CUP')
    ...
    assignment = function() {
      expr = self$addition()
      while(self$match('EQUAL') || self$match('PM') || self$match('CUP')) {
    ...
    addition = function() {
      ...
      while(self$match(c('MINUS', 'PLUS', 'COMMA'))) {
        operator = self$previous()
    ...
    unary = function() {
      if (self$match(c('PLUS', 'MINUS', 'COMMA', self$unary_fns))) {
